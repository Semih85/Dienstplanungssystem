...
{
    public class KayseriOptano : EczaneNobetKisit, IEczaneNobetKayseriOptimization
    {
        ...
        private Model Model(KayseriDataModel data)
        {
            var model = new Model() { Name = "Kayseri Merkez Eczane Nöbet" };
            ...
            foreach (var nobetGrupGorevTip in data.NobetGrupGorevTipler)
            {
                ...
                var altGruplar = data.NobetAltGruplar.Where(w => w.NobetGrupGorevTipId == nobetGrupGorevTip.Id).ToList();
                ...
                foreach (var altGrup in altGruplar)
                {
                    ...
                    for (int i = 0; i < cumartesiGunleri.Count; i++)
                    {
                        var tarih = cumartesiGunleri[i];
                    ...
                    talebiKarsilaAltGrup.EczaneNobetTarihAralikTumu = eczaneNobetTarihAralikGrupBazli.Where(w => w.NobetAltGrupId == altGrup.Id).ToList();
                    talebiKarsilaAltGrup.TalepDetay = altGrup.Adi;
        ...
        public EczaneNobetSonucModel Solve(KayseriDataModel data)
        {
